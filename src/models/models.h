...
struct llama_model_qwen4exp : public llama_model_base {
    ...
    struct graph : public llm_build_delta_net_base {
        graph(const llama_model & model, const llm_graph_params & params);
    protected:
        struct helpers_only_t {};

        // Initialize shared graph helpers without building the target trunk.
        graph(const llama_model & model, const llm_graph_params & params, helpers_only_t);
  ...
  const llama_memory_hybrid_idx_context * mctx_hyb,
        ...
        // Attention over the complete cache window, masked down to the selected cells.
        ...
        // Attention over only the selected cells, gathered into a window per query.
