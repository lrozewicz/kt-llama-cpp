...
class DFlashModel(Qwen3Model):
    ...
    def set_gguf_parameters(self):
        ...
        if "conv_kernel_size" in dflash_config and not self._uses_fork_dflash2_schema:
        ...
        if (self.model_arch != gguf.MODEL_ARCH.GEMMA4_DFLASH_DRAFT
                and softcap is not None and float(softcap) > 0):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if (not self._uses_fork_dflash2_schema
                and (".attention_conv." in name
                     or ".mlp_conv." in name
                     or "model.candidate_selector." in name)):
            raise ValueError(
                "upstream DFlash convolution/selector conversion is not supported; "
                "the DFlash2 wire schema is selected only by DFlash2DraftModel"
            )
...
class DFlash2Model(DFlashModel):
    ...
    model_arch = gguf.MODEL_ARCH.DFLASH
    _uses_fork_dflash2_schema = True
