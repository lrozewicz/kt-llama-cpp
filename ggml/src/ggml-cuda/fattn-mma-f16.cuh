...
#pragma unroll
    ...
    if (np > 1) {
        ...
        // (#27870: all warps hit the same __syncthreads(); only warps with threadIdx.y % np == 0 compute/write.)
...
#pragma unroll
            for (int imeta = 0; imeta < nmeta; ++imeta) {
                KQ_cms[imeta] = expf(meta[imeta].x - KQ_cmn); // KQ combine max scale per warp.
            }
...
static __global__ void flash_attn_ext_f16(
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        if (kb0_start == 0) {
            ...
            flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, needs_fixup, is_fixup, sparse_mask, type_K, type_V>
